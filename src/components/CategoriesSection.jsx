export function CategoriesSection() {
  return (
    <>
      <div className="card-display">
        <div className="card-lorem">
          <img src="src/assets/imag1.jpg" alt="" />
          <p>29.jun.2022</p>
          <h1>Instagram artists with great photography skills</h1>
          <p>
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Incidunt
            porro fuga rerum, ullam,{" "}
          </p>
        </div>
        <div className="card-lorem">
          <img src="src/assets/image2.jpg" alt="" />
          <p>29.jun.2022</p>
          <h1>Instagram artists with great photography skills</h1>
          <p>
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Incidunt
            porro fuga rerum, ullam,{" "}
          </p>
        </div>
        <div className="card-lorem">
          <img src="src/assets/image3.jpg" alt="" />
          <p>29.jun.2022</p>
          <h1>Instagram artists with great photography skills</h1>
          <p>
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Incidunt
            porro fuga rerum, ullam,{" "}
          </p>
        </div>
      </div>
      <div className="card1">
        <div className="card-lorem">
          <img src="src/assets/image4.jpg" alt="" />
          <p>29.jun.2022</p>
          <h1>Instagram artists with great photography skills</h1>
          <p>
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Incidunt
            porro fuga rerum, ullam,{" "}
          </p>
        </div>
        <div className="card-lorem">
          <img src="src/assets/image5.jpg" alt="" />
          <p>29.jun.2022</p>
          <h1>Instagram artists with great photography skills</h1>
          <p>
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Incidunt
            porro fuga rerum, ullam,{" "}
          </p>
        </div>
        <div className="card-lorem">
          <img src="src/assets/image6.jpg" alt="" />
          <p>29.jun.2022</p>
          <h1>Instagram artists with great photography skills</h1>
          <p>
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Incidunt
            porro fuga rerum, ullam,{" "}
          </p>
        </div>
      </div>

      <div class="row">
        <div class="colum">
          <div className="dolor">
            <img src="src/assets/latop.jpg" alt="" className="image" />
            <div className=" minus ">
              <p>29.jun.2022</p>
              <h2>
                {" "}
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo
                minus sequi obcaecati odio laborum ducimus esse non, ea minima
              </h2>
              <p>
                Leather detail shoulder contrastic colour contour stunning
                silhouette working peplum.
              </p>
            </div>
          </div>
          <div className="dolor">
            <img src="src/assets/dramas4.jpg" alt="" className="image" />
            <div className=" minus ">
              <p>29.jun.2022</p>
              <h2>
                {" "}
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo
                minus sequi obcaecati odio laborum ducimus esse non, ea minima
              </h2>
              <p>
                Leather detail shoulder contrastic colour contour stunning
                silhouette working peplum.
              </p>
            </div>
          </div>
          <div className="dolor">
            <img src="src/assets/dramas3.jpg" alt="" className="image" />
            <div className=" minus ">
              <p>29.jun.2022</p>
              <h2>
                {" "}
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo
                minus sequi obcaecati odio laborum ducimus esse non, ea minima
              </h2>
              <p>
                Leather detail shoulder contrastic colour contour stunning
                silhouette working peplum.
              </p>
            </div>
          </div>
        </div>
        <div class="Lorem">
          <h1>
            Lorem ipsum dolor sit amet, consectetur adipisicing elit.
            Reprehenderit dolorum earum maiores corrupti{" "}
          </h1>
        </div>
      </div>
    </>
  );
}
